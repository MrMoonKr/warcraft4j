...
/**
 * 에셋파일
 * {@link CascFile} implementation for a CDN based CASC, 
 * describing a file stored in the CASC.
 *
...
public class CdnCascFile implements CascFile {
    ...
    public CdnCascFile( long hash, String filename, CdnCascContext cascContext ) throws IllegalArgumentException {
        ...
        this.hash = Optional.of( hash )
                ...
                .orElseThrow( () -> new IllegalArgumentException( format(
                        "Unable to initialise a CASC file for filename hash %d, which is not registered in the CASC.",
                        hash ) ) );
        if ( isNotEmpty( filename ) && CdnCascContext.hashFilename( filename ) != hash ) {
            throw new IllegalArgumentException(
                    format( "Filename %s has hash %d while the hashcode %d has been provided.", filename,
                            CdnCascContext.hashFilename( filename ), hash ) );
        }
     ...
     *
     * @return Optional containing the CASC context or empty when the CASC context
     *         has been closed/GC'd.
     */
