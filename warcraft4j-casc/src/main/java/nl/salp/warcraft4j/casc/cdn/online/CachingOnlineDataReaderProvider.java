...
/**
 * {@link DataReaderProvider} for reading files over HTTP, caching the files
 * prior per version of the CASC the file is in.
 * <p>
 * This implementation has no cache-expiration or cleanup functionality and can
 * consume quite a bit of drive space over time.
 * </p>
 *
...
public class CachingOnlineDataReaderProvider implements DataReaderProvider {
     ...
     *
     * @throws IllegalArgumentException When the cache directory is not available
    ...
    public CachingOnlineDataReaderProvider( CdnCascConfig cdnCascConfig, Path cacheRootDirectory ) throws IllegalArgumentException {
        ...
        if ( Files.notExists( cacheDirectory ) ) {
            ...
            try {
                ...
                throw new IllegalArgumentException( format( "Unable to create CDN cache directory %s", cacheDirectory ),
                        e );
            }
        } 
        else {
        ...
        LOGGER.trace( "Created cached CDN data reader for CDN URL {}, using cache directory {}", cdnUrl,
                cacheDirectory );
    }
     ...
     *
     * @throws CascParsingException When caching the file failed or the URL is
     *                              invalid.
     */
     ...
     * @return The path of the cached file.
     *
     * @throws IOException When reading the file or writing the cached version of
     *                     the file fails.
     */
     ...
     *
     * @throws CascParsingException When the provided URL is invalid or does not
    ...
    /**
     * Convert the URL of a file to the path of the cached version (which might not
     * exist).
     *
     * @param url The URL of the file.
     *
