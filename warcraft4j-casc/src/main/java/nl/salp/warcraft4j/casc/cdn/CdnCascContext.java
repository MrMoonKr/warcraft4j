...
/**
 * CDN based CASC context for accessing CASC stored files and their reference
 * trees.
 *
...
public abstract class CdnCascContext {
    ...
    protected CdnCascContext( Warcraft4jConfig warcraft4jConfig ) {
        LOGGER.debug(
                "Created CASC context for branch {}, region {} and locale {} (wow directory: {}, online: {}, caching: {})",
    ...
    protected EncodingFile parseEncoding() throws CascParsingException {
        ...
        LOGGER.debug( "Initialising encoding file (fileKey: {}, fileSize: {}).",
                getCdnCascConfig().getStorageEncodingFileChecksum(), getCdnCascConfig().getStorageEncodingFileSize() );
        try ( DataReader reader = readerSupplier.get() ) {
            EncodingFile encodingFile = new EncodingFileParser().parse( reader,
                    getCdnCascConfig().getExtractedEncodingFileSize() );
            return encodingFile;
        } 
        catch ( IOException e ) {
            throw new CascParsingException( format( "Error parsing encoding file %s",
                    getCdnCascConfig().getExtractedEncodingFileChecksum().toHexString() ), e );
        }
    }
    ...
    public final Optional<EncodingEntry> getEncodingEntry( RootEntry rootEntry ) {
        return Optional.ofNullable( rootEntry ).map( RootEntry::getContentChecksum ).flatMap( this::getEncodingEntry );
    }
    ...
    public final Optional<EncodingEntry> getEncodingEntry( ContentChecksum contentChecksum ) {
        return Optional.ofNullable( contentChecksum ).flatMap( getEncoding()::getEncodingEntry );
    }
    ...
    /**
     * Check if a filename hash refers to registered data in the CASC with all data
     * segment present.
     *
     * @param hash The filename hash.
     *
     * @return {@code true} if the filename hash is registered and all data segments
     *         are present in the CASC.
     */
    public boolean isRegisteredData( long hash ) {
        List<ContentChecksum> contentChecksum = getContentChecksums( hash );
        List<FileKey> fileKeys = contentChecksum.stream().map( this::getFileKey ).filter( Optional::isPresent )
                .map( Optional::get ).collect( Collectors.toList() );
        List<IndexEntry> indexEntries = fileKeys.stream().map( this::getIndexEntry ).filter( Optional::isPresent )
                .map( Optional::get ).collect( Collectors.toList() );
    ...
    public boolean isRegistered( long hash ) {
        return getFilename( hash ).map( StringUtils::isNotEmpty ).orElse( getRootFile().isEntryAvailable( hash ) );
    }

    /**
     * Check if a filename refers to registered data in the CASC with all data
     * segment present.
     *
     * @param filename The filename.
     *
     * @return {@code true} if the filename is registered and all data segments are
     *         present in the CASC.
     */
    public boolean isRegisteredData( String filename ) {
        return getHash( filename ).map( this::isRegisteredData ).orElse( false );
    }
    ...
    /**
     * Get the hash for a filename if the hash refers to an entry available in the
     * context, resolving the hash if possible.
     *
     * @param filename The filename.
     *
     * @return Optional containing the hash of the filename if the hash refers to an
     ...
     * @param contentChecksum The content checksum.
     *
     * @return {@code Optional} of the file key, being empty if none was found for
     *         the checksum.
     */
     ...
     *
     * @return {@code Optional} of the index entry, being empty if none was found
     *         for the file key.
     */
    ...
    /**
     * Get the {@link DataReaderProvider} to use for getting the data readers to
     * use.
     *
     ...
     */
    public Supplier<DataReader> getDataReader( IndexEntry entry )
            throws CascParsingException, CascEntryNotFoundException {
                
        LOGGER.trace( "Getting data reader for file key {} (uri: {}, datafile: {}, offset: {}, size: {})",
                entry.getFileKey().toHexString(), 
                getDataFileUri( entry ).orElse( "#error#" ), 
                entry.getFileNumber(),
                entry.getDataFileOffset(), 
                entry.getFileSize() );

        return Optional.ofNullable( entry )
                .flatMap( this::getDataFileUri )
                .map( file -> getDataReader( file, entry.getDataFileOffset(), entry.getFileSize() ) )
                .orElseThrow( () -> new CascEntryNotFoundException(
                        format( "Unable to get the data reader for entry %s in data file %d", 
                                entry.getFileKey(),
                                entry.getFileNumber() ) ) );
    }
    ...
    protected Supplier<DataReader> getDataReader( String dataFile, long dataFileOffset, long fileSize )
        ...
        LOGGER.trace( "Getting data reader for file key {} (offset: {}, size: {})", dataFile, dataFileOffset,
                fileSize );
     ...
     * @param entry The index entry.
     *
     * @return {@code Optional} of the data file URI, being empty if none was found
     *         for the index entry.
     */
    protected abstract Optional<String> getDataFileUri( IndexEntry entry );

    /**
     * Get a {@link DataReader} for a file extracted from the CASC, referenced by
     * the hashcode of the filename.
     *
     * @param hashCode The hash code of the filename.
     *
     * @return The data reader.
     *
     * @throws CascParsingException       When there is a problem creating a data
     *                                    reader for the file.
     * @throws CascEntryNotFoundException When the files could not be resolved for
     *                                    the hash code.
     */
    public DataReader getFileDataReader( long hashCode ) throws CascParsingException, CascEntryNotFoundException {
        ...
        return Optional.of( hashCode ).map( this::getIndexEntries ).filter( indices -> !indices.isEmpty() )
                .map( this::getFileDataReader ).orElseThrow( () -> new CascEntryNotFoundException(
                        format( "No entry found for a file with hashcode %d", hashCode ) ) );
    }
    ...
    public DataReader getFileDataReader( String filename ) throws CascParsingException, CascEntryNotFoundException {
        ...
        return getHash( filename ).map( this::getFileDataReader ).orElseThrow( () -> new CascEntryNotFoundException(
                format( "No entry found for a file with filename %s", filename ) ) );
    }

    /**
     * Get a {@link DataReader} for a file extracted from the CASC, referenced by
     * the checksum of the file's content.
     *
     * @param contentChecksum The {@link ContentChecksum} containing a checksum of
     *                        the file.
     *
     * @return The data reader.
     *
     ...
     */
    public DataReader getFileDataReader( ContentChecksum contentChecksum )
            throws CascParsingException, CascEntryNotFoundException {
        LOGGER.trace( "Getting DataReader for extracted file with content checksum {}", contentChecksum.toHexString() );
        return Optional.ofNullable( contentChecksum ).flatMap( this::getEncodingEntry )
                .map( EncodingEntry::getFileKeys ).map( this::getIndexEntries ).map( this::getFileDataReader )
                .orElseThrow( () -> new CascEntryNotFoundException(
                        format( "No entry found for a file with content checksum %s", contentChecksum ) ) );
    }

    /**
     * Get a {@link DataReader} for a file extracted from the CASC, referenced by 1
     ...
     *
     * @throws CascParsingException       When the data reader cannot be created for
     *                                    the index entries.
     * @throws CascEntryNotFoundException When there is no data for one or more
     *                                    index entries.
     */
    public DataReader getFileDataReader( List<IndexEntry> indexEntries )
            throws CascParsingException, CascEntryNotFoundException {
        ...
        if ( indexEntries == null || indexEntries.isEmpty() ) {
            ...
            LOGGER.trace( "Getting DataReader for extracted file with file key {}",
                    indexEntries.get( 0 ).getFileKey().toHexString() );
            dataReader = getFileDataReader( indexEntries.get( 0 ) );
        } else {
            LOGGER.trace( "Getting DataReader for extracted file with {} file keys: {}", indexEntries.size(),
                    indexEntries.stream().map( IndexEntry::getFileKey ).map( FileKey::toHexString )
                            .collect( Collectors.joining( "," ) ) );
            List<Supplier<? extends DataReader>> dataReaders = indexEntries.stream()
                    .map( this::getFileDataReaderSupplier ).collect( Collectors.toList() );
    ...
    /**
     * Get a {@link DataReader} for a file, or file segment, extracted from the
     * CASC, referenced by an index entry.
     *
     ...
     * @throws CascEntryNotFoundException When there is no data for the index entry.
     */
    public DataReader getFileDataReader( IndexEntry indexEntry )
            throws CascParsingException, CascEntryNotFoundException {
        if ( indexEntry == null ) {
            throw new CascParsingException( "Cannot create a data reader with no index entries provided." );
        }
        LOGGER.trace( "Getting BLTE data reader for {} (uri: {}, datafile: {}, offset: {}, datasize: {})",
                indexEntry.getFileKey().toHexString(), getDataFileUri( indexEntry ).orElse( "#error#" ),
                indexEntry.getFileNumber(), indexEntry.getDataFileOffset(), indexEntry.getFileSize() );
        return new BlteDataReader( getDataReader( indexEntry ), indexEntry.getFileSize() );
    }

    /**
     * Get a {@link DataReader} for a file, or file segment, extracted from the CASC
     * and validated against a content checksum, referenced by an index entries.
     *
     * @param indexEntry      The index entry referencing the file (segment).
     ...
     * @throws CascEntryNotFoundException When there is no data for the index entry.
     */
    public DataReader getFileDataReader( IndexEntry indexEntry, ContentChecksum contentChecksum )
            throws CascParsingException, CascEntryNotFoundException {
        if ( indexEntry == null ) {
            throw new CascParsingException( "Cannot create a data reader with no index entries provided." );
        }
        LOGGER.trace(
                "Getting BLTE data reader for {} with checksum {} (uri: {}, datafile: {}, offset: {}, datasize: {})",
                indexEntry.getFileKey().toHexString(), contentChecksum.toHexString(),
                getDataFileUri( indexEntry ).orElse( "#error#" ), indexEntry.getFileNumber(),
    ...
    /**
     * Get a supplier for a {@link DataReader} for a file extracted from the CASC,
     * referenced by 1 or more index entries.
     *
     * @param indexEntry The index entry referencing the file (segment).
     *
    ...
    public List<IndexEntry> getIndexEntries( long hashCode ) {
        return Optional.ofNullable( getContentChecksums( hashCode ) )
                .map( c -> c.stream().map( this::getFileKey ).filter( Optional::isPresent ).map( Optional::get )
                        .collect( Collectors.toList() ) )
                .map( c -> c.stream().map( this::getIndexEntry ).filter( Optional::isPresent ).map( Optional::get )
                        .collect( Collectors.toList() ) )
                .orElse( Collections.emptyList() );
    }
