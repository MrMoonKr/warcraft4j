...
/**
 * {@link CascService} implementation for CDN (either online CDN or local
 * installation) based data.
 *
...
public class CdnCascService implements CascService {
    ...
    public Set<CascFile> getAllCascFiles() {
        return cascContext.getHashes().stream().distinct()
                .map( hash -> cascContext.getFilename( hash ).filter( StringUtils::isNotEmpty )
    ...
    public List<RootEntry> getRootEntries( CascFile cascFile ) {
        return Optional.ofNullable( cascFile ).map( CascFile::getFilenameHash ).map( cascContext::getRootEntries )
                .orElse( emptyList() );
    }
    ...
    public Optional<EncodingEntry> getEncodingEntry( RootEntry rootEntry ) {
        return Optional.ofNullable( rootEntry ).flatMap( cascContext::getEncodingEntry );
    }
    ...
    public List<IndexEntry> getIndexEntries( EncodingEntry encodingEntry ) {
        return Optional.ofNullable( encodingEntry ).map( cascContext::getIndexEntries ).orElse( emptyList() );
    }
    ...
    public boolean isDataAvailable( CascFile cascFile ) {
        return Optional.ofNullable( cascFile ).map( CascFile::getFilenameHash ).map( cascContext::isRegisteredData )
                .orElse( false );
    }
    ...
    public boolean isDataAvailable( IndexEntry indexEntry ) {
        return Optional.ofNullable( indexEntry ).map( IndexEntry::getFileKey ).flatMap( cascContext::getIndexEntry )
                .map( e -> e != null ).orElse( false );
    }
    ...
    public DataReader getDataReader( CascFile cascFile ) throws CascEntryNotFoundException {
        return Optional.ofNullable( cascFile ).map( CascFile::getFilenameHash ).filter( cascContext::isRegisteredData )
                .map( cascContext::getFileDataReader )
                .orElseThrow( () -> new CascEntryNotFoundException(
                        format( "No data reader found for casc file with hash %d%s", cascFile.getFilenameHash(),
                                cascFile.getFilename().map( n -> " and name " + n ).orElse( "" ) ) ) );
    }
    ...
    public DataReader getDataReader( IndexEntry indexEntry ) throws CascEntryNotFoundException {
        return Optional.ofNullable( indexEntry ).filter( this::isDataAvailable ).map( cascContext::getFileDataReader )
                .orElseThrow( () -> new CascEntryNotFoundException(
                        format( "No data reader found for index entry with file key %d and file number %d",
