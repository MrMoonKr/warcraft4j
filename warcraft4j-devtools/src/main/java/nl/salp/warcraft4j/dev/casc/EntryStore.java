...
public class EntryStore {
    ...
    public Set<CascEntry> getEntries() {
        return entries.values().stream().filter( e -> e != null ).distinct().collect( Collectors.toSet() );
    }
    ...
    public int getHeaderCount( FileHeader header ) {
        return ( int )headerExtensions.values().stream().map( headerExtensions::getKeys ).flatMap( Set::stream )
                .filter( h -> h.equals( header ) ).count();
    }
    ...
    public int getExtensionCount( String extension ) {
        return ( int )headerExtensions.values().stream().map( headerExtensions::getKeys ).flatMap( Set::stream )
                .filter( ext -> ext.equals( extension ) ).count();
    }
    ...
    public Set<FileHeader> getHeadersForExtension( String extension ) {
        return cleanFilename( extension ).map( headerExtensions::getKeys ).orElse( Collections.emptySet() );
    }

    public Set<String> getExtensionsForHeader( FileHeader fileHeader ) {
        return Optional.ofNullable( fileHeader ).map( headerExtensions::getValues ).orElse( Collections.emptySet() );
    }

    public Set<String> getPathsForExtension( String extension ) {
        return cleanFilename( extension ).map( extensionPaths::getValues ).orElse( Collections.emptySet() );
    }

    public Set<String> getPathsForHeader( FileHeader header ) {
        return headerExtensions.getValues( header ).stream().map( this::getPathsForExtension ).flatMap( Set::stream )
                .distinct().collect( Collectors.toSet() );
    }

    public Set<String> getExtensionsForPath( String path ) {
        return cleanFilename( path ).map( extensionPaths::getKeys ).orElse( Collections.emptySet() );
    }

    public Set<FileHeader> getHeadersForPath( String path ) {
        return cleanFilename( path ).map( extensionPaths::getKeys ).map( Set::stream ).map( stream -> stream
                .map( this::getHeadersForExtension ).flatMap( Set::stream ).distinct().collect( Collectors.toSet() ) )
                .orElse( Collections.emptySet() );
    }

    public static Optional<String> cleanFilename( String filename ) {
        return Optional.ofNullable( filename ).filter( StringUtils::isNotEmpty ).map( String::trim )
                .map( f -> f.replace( '/', '\\' ) ).map( String::toUpperCase );
    }

    public static Optional<String> getExtension( String filename ) {
        return Optional.ofNullable( filename ).flatMap( EntryStore::cleanFilename ).filter( f -> f.contains( "." ) )
                .map( f -> f.substring( f.lastIndexOf( '.' ) ) );
    }

    public static Optional<String> getPath( String filename ) {
        return Optional.ofNullable( filename ).flatMap( EntryStore::cleanFilename ).map( f -> f.replace( '/', '\\' ) )
                .filter( f -> f.contains( "\\" ) ).map( f -> f.substring( 0, f.lastIndexOf( '\\' ) ) );
    }
}
