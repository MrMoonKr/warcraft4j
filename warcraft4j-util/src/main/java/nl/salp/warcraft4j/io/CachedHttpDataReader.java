...
/**
 * {@link DataReader} implementation that (pre-) reads all data from a
 * HTTP-based online file before providing it as a data reader.
 *
...
public class CachedHttpDataReader extends ByteArrayDataReader {
     ...
     * <p>
     * FIXME Currently reads the whole file and passes it with the offset/length to
     * the {@link ByteArrayDataReader}, change to to only read wanted data.
     * </p>
     *
     * @param url    The URL of the file to read.
     * @param offset The file offset to start reading from.
     ...
     *
     * @throws DataReadingException When reading the file failed or the data segment
     *                              is out of bounds.
     */
    private static byte[] getData( String url, long offset, long length ) throws DataReadingException {
        if ( offset < 0 ) {
            throw new DataReadingException(
                    format( "Can't create a http reader for %s with negative data block offset %d.", url, offset ) );
        }
        if ( length < 0 ) {
            throw new DataReadingException(
                    format( "Can't create a http reader for %s with negative data block length %d.", url, length ) );
        }
        byte[] data = getData( url );
        if ( offset + length > data.length ) {
            throw new DataReadingException(
                    format( "Can't create a http reader for %s with %d bytes of data from offset %d with length %d.",
